...
export default function DivorcesTab({ attendanceId }: { attendanceId: string }) {
  ...
  const add = async () => {
    if (!what.trim()) return
    const { error } = await insertEnergyDivorce({ attendance_id: attendanceId, what: what.trim(), reason: reason || null, percentage: percentage ? Math.min(100, Math.max(0, parseFloat(percentage))) : null, result: result || null, notes: notes || null })
    if (error) toast('Erro ao adicionar', 'error')
    else { toast('Corte registrado'); qc.invalidateQueries({ queryKey: ['divorces', attendanceId] }); setAdding(false); setWhat(''); setReason(''); setPercentage(''); setResult(''); setNotes('') }
  }

  const remove = async (id: string) => {
    if (await confirm('Remover este corte energético?')) {
  ...
  return (
    <div>
      ...
      {divorces.length > 0 ? (
        <table>
          <thead><tr><th>Corte realizado</th><th>Motivo</th><th>%</th><th>Resultado</th><th></th></tr></thead>
          <tbody>
            {divorces.map(d => (
              <tr key={d.id}>
                <td>{d.what}</td>
      ...
      ) : <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Nenhum corte energético registrado</p>}

      {adding && (
        <div className="card" style={{ marginTop: 'var(--space-4)' }}>
          <div className="form-grid">
            <Input label="Corte realizado" value={what} onChange={e => setWhat(e.target.value)} required />
