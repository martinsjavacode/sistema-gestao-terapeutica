...
import type { TemplateSection, CustomSectionValue } from '../../services/templates'

interface Props {
  section: TemplateSection
  value: CustomSectionValue | undefined
  onSave: (values: { content?: string; items?: string[]; rating?: number; checked?: boolean }) => void
}

export default function CustomSectionRenderer({ section, value, onSave }: Props) {
  const fieldType = section.field_type ?? 'text'

  switch (fieldType) {
    case 'text':
      return <TextField initialValue={value?.content ?? ''} placeholder={section.config?.placeholder} onSave={v => onSave({ content: v })} />
    case 'list':
      return <ListField initialItems={value?.items ?? []} onSave={items => onSave({ items })} />
    case 'rating':
      return <RatingField initialValue={value?.rating ?? null} maxRating={section.config?.max_rating ?? 10} label={section.config?.rating_label ?? '/10'} onSave={v => onSave({ rating: v })} />
    case 'checkbox':
      return <CheckboxField initialValue={value?.checked ?? false} label={section.config?.checkbox_label ?? 'Realizado'} onSave={v => onSave({ checked: v })} />
    default:
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
      <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
        ...
        <button
          ...
          className="edit-btn"
          style={{ padding: '6px 10px', opacity: newItem.trim() ? 1 : 0.4 }}
          type="button"
      ...
      {items.length === 0 && (
        <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', textAlign: 'center', padding: 'var(--space-2)' }}>
